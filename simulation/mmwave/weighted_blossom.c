		...
		for(j = 0; j<region->vCells;j++) {
			...
			while (aItem != NULL){
				...
				while (nItem != NULL) {
					...
					nCar = (struct vehicle*)aNeigh->carItem->datap;
					g[aCar->match_id][nCar->match_id].w = aNeigh->data_rate;
					nItem = nItem->next;
				}
				aItem = aItem->next;
			}
  ...
  cout<<weight_blossom().first<<'\n';
	...
	for(i = 0; i<region->hCells; i++){       
		for(j = 0; j<region->vCells;j++) {
			...
			while (aItem != NULL){
				aCar = (struct vehicle*)aItem->datap;
				if (match[aCar->match_id]==0) {aCar->role=0; aCar->choose_car_Item = NULL; aItem = aItem->next; continue;}
				aCar->role = 2;
				nItem = aCar->neighbours.head;
				while (nItem != NULL) {
					aNeigh = (struct neighbour_car*)nItem->datap;
					nCar = (struct vehicle*)aNeigh->carItem->datap;
					if (match[aCar->match_id]==nCar->match_id) {
						aCar->choose_car_Item = aNeigh->carItem;
						aItem = aItem->next;
						continue;
					}
					nItem = nItem->next;
