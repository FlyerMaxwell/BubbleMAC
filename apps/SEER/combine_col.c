...
int main(int argc, char**argv)
{
	...
	int i, j, rId, rDi, atS2B, atB2S, at, nSlides, first=1, files, nRoads, count;
	...
	if(argc < 4) {
		printf("Usage: %s nSlides nRoads dumpfile1 dumpfile2 ...\n", argv[0]);
		exit(0);
	}
	nSlides=atoi(argv[1]);
	nRoads=atoi(argv[2]);
	...
	for(i=0;i<nSlides*files;i++) {
		bRoad = roadlist;
		count = nRoads;
		while ((count==-1||count>0)&&bRoad != NULL) {
			if(bRoad->direction ==0) {
				whichScrap = bRoad->aRoad->scraps;
				while(whichScrap!=NULL) {
					if((whichScrap->slidesS2B)[i].condition==-1)
						printf("NaN\t");
					else
						printf("%0.2lf\t", (whichScrap->slidesS2B)[i].condition);
					whichScrap = whichScrap->next;
				}
			}
			if(bRoad->direction ==1) {
				whichScrap = bRoad->aRoad->scraps;
				while(whichScrap!=NULL) {
					if((whichScrap->slidesB2S)[i].condition==-1)
						printf("NaN\t");
					else
						printf("%0.2lf\t", (whichScrap->slidesB2S)[i].condition);
					whichScrap = whichScrap->next;
				}
			}
			bRoad = bRoad->next;
