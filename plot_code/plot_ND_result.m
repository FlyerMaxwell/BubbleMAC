...
for car_num = traffic_density
    ...
    for pro_index = pro
        ...
        for i=0:1:99
            pro_num = pro_index+i
            path1 = sprintf('/home/sjg/vanet1.0_old/vanet1.0/simulation_result/Neighbour_Discovery/density_%d_%d.csv', car_num, pro_num);
            fid1 = fopen(path1,'rt');
            C1 = textscan(fid1, '%f');
            fclose(fid1);
            data1 = deal(C1{1});
    ...
    for i=1:1:4
         mat_plot = [mat_plot;raw_data(20+i*20,:)];
    end
...
% 
% 
    ylabel('Network Throughput(Gbps)','FontSize',20);
    ...
    e11 = errorbar(X-0.145, mat_plot(:,1), mat_plot(:,2), mat_plot(:,3),'k', 'Linestyle', 'None', 'LineWidth', 2);
    e12 = errorbar(X+0.145, mat_plot(:,4), mat_plot(:,5), mat_plot(:,6),'k', 'Linestyle', 'None', 'LineWidth', 2);
  
    legend(p1,'COMNET','Random','Orientation','vertical','Location','NorthWest','Fontsize',16);
    lineWidth=1.5
    capSize =5
...
%     
     ...
     save_path2 = sprintf('NDcompare_%d', car_num);
